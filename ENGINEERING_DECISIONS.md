...
| D-E29 | Ticket/UID format (E1-02 A13, supersedes the digits-only reading) | **Free-form text**, not digits-only: `Entry.ticketNumber` / `RetiredTicket.ticketNumber` are `VARCHAR(64)`. Accepted = any non-empty printable text after trim, ≤ 64 chars; only C0/C1 control characters are refused (they signal a mangled file or a wrong column mapping). Uniqueness and the never-reuse ledger are **case-SENSITIVE** — `abc` and `ABC` are distinct tickets — and leading zeros are significant (`007` ≠ `7`). Applies to BOTH entry paths: CSV import and the individual add form. Operator-facing lists sort with a numeric-aware collator (`src/lib/ticket.ts`) so text ordering still reads `1, 2, 10`; the eligible pool keeps its plain DB order, since pool order does not affect draw fairness. |
